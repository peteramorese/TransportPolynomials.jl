...
target_region = Hyperrectangle(low=[-0.3, -0.2, -0.2, 0.1], high=[0.3, 0.1, 0.2, 0.3])
duration = 2.5
...
# Compute upper bounds (mean + bound)
euler_prob_upper = [min(p + b, 1.0) for (p, b) in zip(euler_prob_traj, hoeffding_bounds)]

# Plot the flowpipe
plt_fp = plot(aspect_ratio = :equal)
plt_fp = plot_flowpipe!(plt_fp, flow_pipe; vars=(1, 3), color=:teal, alpha=0.0, n=10)
plt_fp = plot_2D_region!(plt_fp, target_region_u; vars=(1, 3), color=:red, alpha=0.8)
...
if save_plots
    ...
    savefig(plt_prob, joinpath(figures_dir, "cartpole_4D_probability.pdf"))
