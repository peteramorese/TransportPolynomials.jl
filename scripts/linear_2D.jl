...
# Specifications #
A = diagm([-0.5, -1.0])
true_system, dtf = linear_system(A=A)
target_region = Hyperrectangle(low=[-0.3, -0.3], high=[0.2, 0.2])
duration = 2.2
model_degrees = 8 * ones(Int, dimension(true_system))
fp_deg = 5
...
X, fx_hat = generate_data(true_system, 4000; domain_std=1.5, noise_std=0.00)
...
function sampler()
    return [rand(Normal(0.0, 0.4)) for i in 1:2]
end
...
truth_prob = [eval_lin_sys(A, [0.0, 0.0], diagm([0.4, 0.4].^2), t, target_region) for t in timestamps]
