struct TensorErrorVariables
    E::Vector{Any} # Array of polynomial variable tensors
end

function create_tensor_error_variables(dimension::Int, max_order::Int; basename::Symbol=:ϵ)
    E = Vector{Any}(undef, max_order)
    
    for order in 1:max_order
        # Generate all index combinations
        indices = CartesianIndices(ntuple(_ -> n, order))
        
        # Create polyvars with names like x_i, x_ij, x_ijk, etc.
        vars = Array{PolyVar{true}}(undef, size(indices))
        for idx in indices
            idx_tuple = Tuple(idx)
            name = Symbol(basename, '_', join(idx_tuple, '_'))
            vars[idx...] = @polyvar(name)
        end
        E[order] = vars
    end
    
    return TensorErrorVariables(E)
end
